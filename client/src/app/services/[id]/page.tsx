...
export default function DetailsPetService() {
  ...
  const petshopServiceShowQuery = useQuery({
    ...
    queryFn: () => fetchPetshopService(id || ""),
  });

  if (petshopServiceShowQuery.isLoading) {
    return (
  ...
  if (petshopServiceShowQuery.isError) {
    return (
      <div>
        Đã xảy ra lỗi:{" "}
        {petshopServiceShowQuery.error instanceof Error
          ? petshopServiceShowQuery.error.message
          : "Unknown error"}
      </div>
    );
  }
