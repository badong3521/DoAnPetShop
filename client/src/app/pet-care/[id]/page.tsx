...
export default function DetailsPetCare() {
  const searchParams = useSearchParams();
  const id = searchParams.get("id");

  const petDiseaseShowQuery = useQuery({
    queryKey: [PET_DISEASE_KEY, id],
    queryFn: () => fetchPetDisease(id || ""),
  });

  if (petDiseaseShowQuery.isLoading) {
    return (
  ...
  if (petDiseaseShowQuery.isError) {
    return (
      <div>
        Đã xảy ra lỗi:{" "}
        {petDiseaseShowQuery.error instanceof Error
          ? petDiseaseShowQuery.error.message
          : "Unknown error"}
      </div>
    );
  }
          ...
          <div className="container mx-auto ">
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="flex gap-4 justify-between items-end mb-4">
                ...
                <p className="text-xl">
                  ...
                  <span className="text-orange-500 font-bold">
                    {petDiseaseShowQuery?.data?.severity}
                  </span>
                </p>
              </div>
              <div className="flex justify-center mb-14 gap-y-8 lg:gap-y-0 flex-wrap md:flex-wrap lg:flex-nowrap lg:flex-row lg:justify-between lg:gap-x-8">
                ...
                >
                  {petDiseaseShowQuery?.data?.description}
                </ReactMarkdown>
              </div>
            </div>
          </div>
